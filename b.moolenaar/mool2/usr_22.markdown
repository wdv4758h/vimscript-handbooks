Finding the file to edit^[For Vim version 7.4.  Last change: 2012 Nov 15]
========================
...
Try using `<Enter>` while the cursor is atop a directory name.  The
...
The argument of `":hide"` is another command.  `":hide"` makes that
